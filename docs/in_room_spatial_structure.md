...
This document describes how to add **structured intra-room spatial modeling** to your existing Python-based text adventure engine. The design follows the previously discussed **intra-room movement** and **structured room graph** models while remaining fully compatible with your engine’s current architecture—dataclasses (`Location`, `Item`, `Actor`), JSON game state, property-based extensibility, and `StateAccessor` as the authoritative query/update interface.
