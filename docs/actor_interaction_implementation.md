...
| 10 | Integration Testing | All | 3-4 days |
...
1. **All unit tests pass** for each phase module
2. **All integration tests pass** for all 8 use cases
3. **Turn phases fire correctly** after successful commands
4. **Conditions progress** each turn with damage and duration
5. **Environmental effects apply** based on part properties
6. **NPCs take actions** including attacks when hostile
7. **Treatment works** via item properties
8. **Services execute** with trust discounts
9. **Pack coordination works** with disposition sync
10. **No code changes needed** for use case scenarios - all property-driven
