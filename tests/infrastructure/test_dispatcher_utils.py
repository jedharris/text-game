...
from examples.big_game.game_behaviors.shared.infrastructure.dispatcher_utils import (
    clear_handler_cache,
    dispatch_or_process,
    load_handler,
)
...
class TestLoadHandler(unittest.TestCase):
    ...
    def test_load_handler_invalid_function(self) -> None:
        ...
        result = load_handler("examples.big_game.game_behaviors.shared.infrastructure.dispatcher_utils:nonexistent_func")
        self.assertIsNone(result)

    def test_load_handler_valid_path(self) -> None:
        """Valid handler path returns function."""
        result = load_handler("examples.big_game.game_behaviors.shared.infrastructure.dispatcher_utils:load_handler")
        self.assertIsNotNone(result)
        self.assertEqual(result, load_handler)

    def test_load_handler_caching(self) -> None:
        """Handler is cached after first load."""
        path = "examples.big_game.game_behaviors.shared.infrastructure.dispatcher_utils:load_handler"
...
class TestDispatchOrProcess(unittest.TestCase):
    ...
    def test_dispatch_to_handler(self) -> None:
        ...
        with patch(
            "examples.big_game.game_behaviors.shared.infrastructure.dispatcher_utils.load_handler",
