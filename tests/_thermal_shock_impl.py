...
def _setup_paths():
    """Ensure big_game directory is in sys.path for behaviors imports."""
    # Remove CWD ('') from path
    while '' in sys.path:
        sys.path.remove('')

    # Add big_game directory first (for behaviors imports)
    big_game_str = str(BIG_GAME_DIR)
    if big_game_str not in sys.path:
        sys.path.insert(0, big_game_str)
