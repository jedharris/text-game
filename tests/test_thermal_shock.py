...
class TestThermalShock(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Load combat module from behavior_libraries
        behavior_lib = project_root / "behavior_libraries"
        if str(behavior_lib) not in sys.path:
            sys.path.insert(0, str(behavior_lib))
