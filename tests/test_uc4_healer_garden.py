...
class TestUC4ToxicPlant(BaseTestCase):
...
class TestUC4Knowledge(BaseTestCase):
...
class TestUC4CureService(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.engine = GameEngine(GAME_DIR)
        self.player = self.engine.game_state.get_actor(ActorId('player'))
        self.healer = self.engine.game_state.get_actor(ActorId('npc_healer'))
        self.gold = None
        for item in self.engine.game_state.items:
...
class TestUC4TeachService(BaseTestCase):
...
class TestUC4TrustDiscount(BaseTestCase):
